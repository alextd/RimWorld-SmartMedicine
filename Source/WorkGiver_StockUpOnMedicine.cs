...
namespace SmartMedicine
{
	...
	{
		public static Dictionary<Pawn, Dictionary<ThingDef, int>> stockUpSettings = new Dictionary<Pawn, Dictionary<ThingDef, int>> ();

		public static Dictionary<ThingDef, int> StockUpSettings(this Pawn pawn)
		{
			if (!stockUpSettings.TryGetValue(pawn, out Dictionary<ThingDef, int> settings))
				stockUpSettings[pawn] = settings = new Dictionary<ThingDef, int>();
			return settings;
		}
		...
		{
			Log.Message(pawn + " IsAtFullStock? ");
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			foreach (ThingDef td in stockable)
			{
				...
				if (Widgets.ButtonInvisible(iconRect))
				{
					if (!doIt)
					{
						Log.Message("SETTING " + td + " to 1");
						pawn.SetStockCount(td, 1);
					}
					else
					{
						Log.Message("REMOVING " + td);
